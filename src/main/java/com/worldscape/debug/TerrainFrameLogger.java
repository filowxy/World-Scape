...
public class TerrainFrameLogger {
    ...
    public static void logChunkTerrainFrame(int chunkX, int chunkZ, RegionController controller, int[][] heightMap, TerrainType[][] typeMap, RegionController.TerrainBlendResult[][] blendMap) {
        ...
        if (!typeDistribution.isEmpty()) {
            ...
            typeDistribution.entrySet().stream().sorted(Map.Entry.<TerrainType, Integer>comparingByValue().reversed()).limit(5L).forEach(e -> typeStr.append(e.getKey().getId()).append(":").append(e.getValue()).append(", "));
            if (typeStr.length() > 7) {
    ...
    public static void logRegionOverview(ControlPointRegion region) {
        ...
        typeCounts.entrySet().stream().sorted(Map.Entry.<TerrainType, Integer>comparingByValue().reversed()).forEach(e -> summary.append(e.getKey().getId()).append(":").append(e.getValue()).append(" "));
